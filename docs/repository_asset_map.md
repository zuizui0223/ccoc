...
Read [portability core v1](portability_core_v1.md) before extending the structural core and [research priorities](research_priorities.md) before opening a new branch.

## Classification rule

An asset belongs to the structural program only when it changes one canonical claim:
...
Identifiability assets ask what finite evidence or retained mechanisms can justify. Legacy assets begin only after a structural contract is fixed.
...
Relay trees are sharpness witnesses. `addressable_completion_bounds.py` is a finite product helper; the sole public lower-bound statement is in `extension_compression_noncommutation.py`.

## B. Selected post-v1 structural extension: non-nested replacement

This is the only authorized post-v1 direction. It replaces nested embeddings by declared total transports that may be many-to-one or one-to-many.

| Asset | Present role |
|---|---|
| `non_nested_portability.py` | edge preservation, transported target exact factorization, and newly legal-word obstruction |
| `non_nested_conservative_transport.py` | conservative schema construction across target-only legal-action growth |
| `tests/test_non_nested_portability.py` | N1/N2 positive, rejection, connectedness, obstruction, and facade regressions |
| `tests/test_non_nested_conservative_transport.py` | N3 positive plus old-action closure and new-action uniformity rejections |
| `scripts/verify_non_nested_replacement_portability.py` | deterministic replay of relations, derived target labels, conservative schema, and obstruction |
| `.github/workflows/non-nested-replacement-portability.yml` | path-scoped test and replay CI |
| `docs/non_nested_replacement_portability.md` | theorem ladder, domain, verification contract, and non-claims |

The N3 construction starts with an exact source projection and a total relation. Old source-legal actions must remain legal and successor-closed; target-only actions must be uniformly available and macro-successor deterministic in each derived target fiber. The certificate then derives a target projection and one schema. It does not cover nonuniform new actions.
...
No new feature or theorem work is allowed here without an explicit dependency from the selected structural or companion question.
...
Certificate manifests, transcript registries, signed checkpoints, coverage adapters, and tiered artifacts preserve provenance. Keep them passing, but add no feature without a publication-grade need.
...
`causal_model.current_theory` is a historical compatibility aggregate and must not receive new exports.

## Current priority order

1. Preserve the core/companion/legacy boundary.
2. Treat N1–N3 plus the fiber-split obstruction as the frozen finite deterministic non-nested package.
3. Do not start approximate, candidate-dependent, or empirical-instantiation branches in parallel.
4. Add new mathematics only by changing a canonical claim, not by adding another special-case witness or panel.
