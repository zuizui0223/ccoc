...
| `REC-26` | Is candidate-set completeness merely bookkeeping for the last-driver theorem? | **REFUTED.** An omitted latent target route can make a declared forced-ON conclusion false. | `docs/failure_mode_audits.md` omitted-latent-competitor audit | Candidate-universe adequacy is an identifying assumption, not a cosmetic metadata condition. |
| `REC-27` | Can a reported NULL be treated as deterministic evidence that every declared driver is OFF despite measurement error? | **REFUTED; risk quantified.** In the two-driver benchmark with false-NULL rate `eps`, false-necessity risk is `eps/(1+2eps)` under the declared equal-prior setup. | `docs/failure_mode_audits.md`; observation-channel modules | Faithful NULL semantics require an observation model or justified power contract. |
