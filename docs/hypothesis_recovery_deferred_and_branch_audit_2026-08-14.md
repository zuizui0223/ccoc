...
| `REC-50` | Is the fixed four-symbol primitive action alphabet `{0,1,fire,tick}` **minimal** among bounded-local one-action-opening realizations with the same exact separation? | **UNRESOLVED / NON-PRIORITY.** The constant-alphabet construction proves four symbols suffice; it explicitly does not prove minimality. | `docs/constant_alphabet_relay.md`; fixed-regular strengthening | Resource-minimality is separate from the established constant-alphabet existence, maximal memory inflation, and logarithmic-access results. Do not call four symbols optimal. |
...
After the suspicious-branch check and the previously completed closed-unmerged-PR audit, no additional branch-only active scientific hypothesis has been found beyond the recovered rows. `REC-40`, `REC-41`, `REC-43`, and `REC-50` come from substantive published scope/inference notes, not from dangling branch names.